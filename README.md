...
[template image](https://developer.apple.com/documentation/appkit/nsimage/istemplate): macOS
ignores its colors and tints the shape from the alpha channel, so it follows a light or dark
menu bar the same way the system clock and Control Center icons do. This is Apple's own naming
convention, so no config option is needed, and icons can be mixed freely - only the ones named
`...Template` are tinted.
